...
def run_subject(subject_id, mesh_dir_base, output_dir, melanin_condition='fair'):
    ...
    try:
        ...
        labrum_labels    = []   # no labrum in lumbar spine

        # Nucleus pulposus: dilate disc annulus inward to fill nucleus space.
        # Uses label 14 (same as synovial fluid - water-like optical props).
...
def build_label_volume(tissues, res, spacing):
    ...
    Orientation check: L3 (label 1) should be SUPERIOR to S1 (label 4).
    If L3_z_mean < S1_z_mean the Z axis is flipped.
    """
    all_verts      = []
    L3_z_mean = None
    S1_z_mean = None
    ...
    for name, (path, label, _) in tissues.items():
        if path is not None:
            ...
            if 'L3-bone' in name:
                L3_z_mean = m.vertices[:, 2].mean()
            elif 'S1-bone' in name:
                S1_z_mean = m.vertices[:, 2].mean()

    z_flip = False
    if AUTO_ORIENT and L3_z_mean is not None and S1_z_mean is not None:
        # L3 should sit superior (higher Z) than S1
        if L3_z_mean < S1_z_mean:
            z_flip = True
            print(f"  [ORIENT] Z-axis inversion detected: "
                  f"L3_z={L3_z_mean:.1f} < S1_z={S1_z_mean:.1f} "
...
def find_joint_line_z(vol, tissues, origin, spacing, mesh_center):
    disc_labels = [t[1] for name, t in tissues.items() if 'disc'    in name]
    nuc_labels  = [t[1] for name, t in tissues.items() if 'nucleus' in name]
    target_mask = np.isin(vol, disc_labels + nuc_labels)
    if target_mask.sum() == 0:
        print("  [JLINE] No disc/nucleus voxels - using Z=0")
        return 0.0
...
def results_to_csv(all_results, output_path="MC_Analysis_LowerBack_650nm.csv",
                   treatment_times_s=(300, 600, 900)):
    import csv
    GROUPS = {
...
def melanin_comparison_to_csv(all_condition_results, output_path, wavelength_nm,
    ...
    with open(output_path, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow([f'MELANIN CONDITION COMPARISON - Lower Back {wavelength_nm} nm'])
        writer.writerow([])
